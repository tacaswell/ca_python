...
def casput(PV_name, value, update=True):
    ...
    start_server()
    if not PV_name in list(PVs.keys()):
        PVs[PV_name] = PV_info()
    PV = PVs[PV_name]
    if PV_value(PV_name) != value or update:
        PV_set_value(PV_name, value)
...
def PV_set_value(PV_name, value):
    ...
    if DEBUG:
        debug("converted %r to %r" % (PV_name, value))
    for object, name in registered_objects:
        if PV_name.startswith(name + "."):
            attribute = PV_name[len(name + ".") :]
...
def notify_subscribers(PV_name):
    ...
    if value is None:
        if DEBUG:
            info("CA: notify_subscribers: %s=%r" % (PV_name, value))
        ##delete_PV(PV_name)
        return
    for address in list(PV.subscribers.keys()):
...
def command_name(command_code):
    """'VERSION', 'EVENT_ADD',.... """
    if not command_code in list(commands.values()):
...
def type_name(data_type):
    ...
    if not data_type in list(types.values()):
...
def process_message(address, request):
    ...
    if command == "SEARCH":
        ...
        if not PV_exists(channel_name):
            return
        if not channel_name in list(PVs.keys()):
        ...
        if PV.channel_SID == None:
            ...
            PV.channel_SID = 1
            while PV.channel_SID in channel_SIDs:
                PV.channel_SID += 1
        reply = message(
...
def value(data_type, data_count, payload):
    ...
    if data_type.endswith("STRING"):
        ...
        if data_count > old_div(len(payload), 2):
            data_count = max(old_div(len(payload), 2), 1)
        payload = payload.ljust(2 * data_count, "\0")
        value = list(unpack(">%dh" % data_count, payload[0 : 2 * data_count]))
        if len(value) == 1:
            value = value[0]
    elif data_type.endswith("FLOAT"):
        if data_count > old_div(len(payload), 4):
            data_count = max(old_div(len(payload), 4), 1)
        ...
        if data_count > old_div(len(payload), 1):
            data_count = max(old_div(len(payload), 1), 1)
        ...
        if data_count > old_div(len(payload), 8):
            data_count = max(old_div(len(payload), 8), 1)
...
def convert(PV_name, value):
    ...
    if new_value != value:
        if DEBUG:
            debug("converting %r from %r to %r" % (PV_name, value, new_value))
    return new_value
...
def nan_equal(a, b):
    ...
    if (
        not isinstance(a, basestring)
        and not isinstance(b, basestring)
...
def nan_equal_2(a, b):
    ...
    value = True
    if DEBUG:
        debug("nan_equal(%r,%r): %r" % (a, b, value))
    return value
